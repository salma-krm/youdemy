            ...
            <div class="card-body table-responsive">
                                ...
                                <td class ="text-end">
                                    ...
                                    <button class="btn d-inline-flex btn-sm btn-primary mx-1" 
                                       type="button"
                                        ...
                                        data-bs-target="#editModal_<?= $cours->getId() ?>">
                                     <span class="pe-2">
             ...
             foreach ($courses as $cours): ?>
                 <div class="modal fade" 
                      id="editModal_<?= $cours->getId() ?>" 
                      tabindex="-1" 
                      aria-labelledby="editModalLabel_<?= $cours->getId() ?>" 
                      aria-hidden="true">
                     <div class="modal-dialog">
                         <div class="modal-content">
                             <div class="modal-header">
                                 <h1 class="modal-title fs-5" id="editModalLabel_<?= $cours->getId() ?>">
                                     Modifier le Cours: <?= htmlspecialchars($cours->getTitre()) ?>
                                 </h1>
                                 <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                             </div>
                             <div class="modal-body">
                                 <form method="POST" action="?route=updatecours&id=<?= $cours->getId() ?>">
                                     <div class="mb-3">
                                         <label for="titre_<?= $cours->getId() ?>" class="form-label">Titre</label>
                                         <input type="text" 
                                                class="form-control" 
                                                name="titre" 
                                                id="titre_<?= $cours->getId() ?>" 
                                                value="<?= htmlspecialchars($cours->getTitre()) ?>" 
                                                required>
                                     </div>
                                     <div class="mb-3">
                                         <label for="contenu_<?= $cours->getId() ?>" class="form-label">Contenu</label>
                                         <input type="text" 
                                                class="form-control" 
                                                name="contenu" 
                                                id="contenu_<?= $cours->getId() ?>" 
                                                value="<?= htmlspecialchars($cours->getcontenu()) ?>" 
                                                required>
                                     </div>
                                     <div class="mb-3">
                                         <label for="description_<?= $cours->getId() ?>" class="form-label">Description</label>
                                         <textarea class="form-control" 
                                                   name="description" 
                                                   id="description_<?= $cours->getId() ?>" 
                                                   rows="3" 
                                                   required><?= htmlspecialchars($cours->getDescription()) ?></textarea>
                                     </div>
                                     <div class="mb-3">
                                         <label for="categorie_<?= $cours->getId() ?>" class="form-label">Catégorie</label>
                                         <select class="form-select" 
                                                 name="categorie_id" 
                                                 id="categorie_<?= $cours->getId() ?>" 
                                                 required>
                                    ...
                                    <?php endif; ?>
                                         ...
                                         <select class="form-multi-select form-select" name="tags[]" multiple>
                                             ...
                                             <?php foreach ($tags as $tag): ?>
                                                 <option value="<?= $tag->id ?>" 
                                                         <?= in_array($tag->id, array_column($cours->getTag(), 'id')) ? 'selected' : '' ?>>
                                                     <?= ($tag->name) ?>
                                                 </option>
                                             <?php endforeach; ?>
                                         </select>
